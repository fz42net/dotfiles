return {
  ...
  config = function()
    require("obsidian").setup({
      ...
      templates = {
        ...
        substitutions = {
          url = function(ctx)
            return vim.g.obsidian_article_url or ""
          end,
          url_title = function(ctx)
            local url = vim.g.obsidian_article_url
            if not url or url == "" then
              return ""
            end

            local html = vim.fn.system({ "curl", "-Ls", url })
            if vim.v.shell_error ~= 0 or not html or html == "" then
              return url
            end

            local title = html:match("<title>(.-)</title>") or html:match("<TITLE>(.-)</TITLE>")
            if not title then
              return url
            end
  ...
  keys = {
    { "<leader>zd", "<cmd>ObsidianToday<cr>", desc = "Open/Create daily note", mode = { "n" } },
    {
      "<leader>za",
      function()
        -- 1. Ask for the URL
        local url = vim.fn.input("Article URL: ")
        if url == nil or url == "" then
          return
        end

        -- Make URL available to template substitutions ({{url}}, {{url_title}})
        vim.g.obsidian_article_url = url

        -- 2. Fetch HTML with curl
        local html = vim.fn.system({ "curl", "-Ls", url })

        -- 3. Extract <title>...</title> (very simple heuristic)
        local title
        if vim.v.shell_error == 0 and html and html ~= "" then
          title = html:match("<title>(.-)</title>") or html:match("<TITLE>(.-)</TITLE>")
        end

        -- 4. Fallback if no title found
        if not title or title == "" then
          title = vim.fn.input("Page title (fallback): ", url)
          if not title or title == "" then
            return
          end
        end

        -- 5. Slugify for filesystem-safe path
        local slug = title:lower()
        -- keep only word chars, spaces, and hyphens
        slug = slug:gsub("[^%w%s-]", "")

        -- collapse any run of spaces and/or hyphens into a single hyphen
        slug = slug:gsub("[%s-]+", "-")

        -- trim leading/trailing hyphens
        slug = slug:gsub("^%-+", ""):gsub("%-+$", "")

        -- 6. Prefix with YYYYMMDD-
        local date_prefix = os.date("%Y%m%d-")
        local note_name = date_prefix .. slug

        -- 7. Create the note from the 'article' template without picker
        --    :Obsidian new_from_template [TITLE_OR_PATH] [TEMPLATE_NAME]
        local cmd = string.format("Obsidian new_from_template %q %s", note_name, "article")
        vim.cmd(cmd)
      end,
      desc = "Create new article note",
      mode = { "n" },
    },
